...
export default function AllWebs() {
    const [det, setDet] = useState({})
    useEffect(()=>{
        async function init(){
            setDet(await commonApi('/global/get-series'))
        }
        init()
    }, [])

    async function searchWeb(){
        setDet(await commonApi('/global/search-series/'+document.getElementById('searchField').value))
    }
    return(
        <>
            <div className="d-flex justify-content-center pb-5">
                ...
                <i className="fa fa-search btn btn-info" onClick={()=>{ searchWeb() }} ></i>
            </div>
            <Table singleLine>
                <Table.Header>
                    <Table.Row>
