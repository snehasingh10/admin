...
export default function WebScrapper() {
    async function init(e){
        e.preventDefault()
        const det = (await req.get('https://api.themoviedb.org/3/tv/'+ document.getElementById("idForScrapp").value +'?api_key='+tmdbApi))
        if(det.data !== undefined){
            document.getElementById('title').value = det.data.name
